...
const HorizontalProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div
      className={`relative ${isConnected ? "group/child-card" : "group/connected-block"}`}
    >
      {isConnected ? (
      ...
      <div
        className={`${isConnected ? "relative ml-8 pt-3" : "group/parent-card"}`}
      >
        <Card
          className={`relative flex justify-between gap-8 border p-4 shadow-none transition hover:border-primary/30 ${isConnected ? "ml-8" : ""}`}
        >
          <CardHeader
            className={`relative flex flex-row items-start ${isSmall ? "gap-4" : "gap-8"} p-0`}
          >
            <ProductImage
              className={`rounded-lg bg-white ${isSmall ? "!h-14 !w-14" : "!h-36 !w-36"}`}
              src={imageSrc}
            />

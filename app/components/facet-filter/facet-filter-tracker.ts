...
export class FacetFilterTracker {
    ...
    update(searchResult: SearchQuery['search'],
    ...
    private groupFacetValues(withoutFilters: SearchFacetValuesQuery['search'],
                             current: SearchQuery['search']['facetValues'] | null,
                             activeFacetValueIds: string[]): FacetWithValues[] {
        if (!current) {
            return [];
        }
        const facetMap = new Map<string, FacetWithValues>();
        for (const {facetValue: {id, name, facet}, count} of withoutFilters.facetValues) {
            ...
            if (facetFromMap) {
                ...
                facetMap.set(facet.id, {id: facet.id, name: facet.name, values: [{id, name, selected}]});
            }
        }
